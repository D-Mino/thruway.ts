...
import { filter, flatMap, merge, take, takeUntil, tap } from "rxjs/operators";
...
export class TopicObservable<EventMsg> extends Observable<any> {
  ...
  public _subscribe(
    ...
    const subscribeMsg = new SubscribeMessage(
      requestId,
      this.options,
      this.uri
    );
    ...
    const subscribedMsg = this.messages.pipe(
      filter(
        (msg: IMessage) =>
          msg instanceof SubscribedMessage && msg.requestId === requestId
      ),
      take(1)
    );
    ...
    const errorMsg = this.messages.pipe(
      ...
      flatMap((msg: ErrorMessage) =>
        Observable.throw(new WampErrorException(msg.errorURI, msg.args))
      ),
      take(1)
    );
    ...
    const unsubscribedMsg = this.messages.pipe(
      filter(
        (msg: IMessage) =>
          msg instanceof UnsubscribedMessage && msg.requestId === requestId
      ),
      take(1)
    );
    ...
    const sub = subscribedMsg
      .pipe(
        tap((m: any) => (subscriptionId = m.subscriptionId)),
        flatMap((m: SubscribedMessage) => {
          ...
          return this.messages.pipe(
            filter(
              (msg: IMessage) =>
                msg instanceof EventMessage && msg.subscriptionId === sid
            )
          );
        }),
        merge(errorMsg),
    ...
    disposable.add(() => {
      ...
      const unsubscribeMsg = new UnsubscribeMessage(
        Utils.uniqueId(),
        subscriptionId
      );
